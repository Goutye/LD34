...
function Round:initialize(slice)
	...
	self.roundInfo = "Protect yourself!"
	self.isStart = true
	self.timeStart = 0
	self.timeStartMax = 2
end
...
function Round:draw()
	...
	if self.isEnd then
		...
		local box2 = EasyLD.box:new(self.polyRound.x + 100, self.polyRound.y + 10, EasyLD.window.w/3 - 100, EasyLD.window.h/3-10)
