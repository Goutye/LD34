...
local RoundTitle = require 'rounds.RoundEnd'
...
function EndScreen:changeColor()
...
function EndScreen:update(dt)
	...
	if EasyLD.mouse:isPressed("l") then
		EasyLD.screen:nextScreen(TitleScreen:new(), "fusion", nil, 0.5, false, "quadinout")
	end
end
...
function EndScreen:draw()
	...
	self.rounds[self.currentRound]:draw(self.gamedata)
...
function EndScreen:oneIsDead()
	self.nbAIs = self.nbAIs - 1
end

function EndScreen:getRoundEntities()
	return self.rounds[self.currentRound].entities
end

function EndScreen:getSliceEntities()
	return self.slice.entities
end

function EndScreen:getTop()
